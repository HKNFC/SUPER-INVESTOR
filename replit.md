...
- `data_fetcher.py` — Orchestration layer: selects provider or mock data, exposes reusable functions (latest price, history, returns, 52w high, avg volume). `fetch_fundamentals(symbol, market)` has dual-provider fallback: tries Twelve Data `/statistics` first, auto-switches to Yahoo Finance on "pro plan required" error via `_yahoo_fundamentals_mode` flag (one-time detection, all subsequent calls routed to Yahoo). Supports `skip_fundamentals` parameter to bypass fundamentals entirely when quality filter is off. Logs financial data coverage per market. Provides all standalone technical indicator functions (`calculate_moving_averages` (MA20/MA50/MA200 + ratios), `calculate_rsi`, `calculate_macd` (line/signal/histogram), `calculate_atr`, `calculate_volume_ratio`, `calculate_obv`, `calculate_mfi`). The `build_technical_data` pipeline computes all 21 indicator columns from OHLCV in one pass and attaches them to the master DataFrame
- `yahoo_provider.py` — Yahoo Finance fallback provider using yfinance. `fetch_yahoo_history` for OHLCV data, `fetch_yahoo_fundamentals` for financial data (revenue, net_income, equity, margins, ROE, ROA, ROIC, PE, PB, EV/EBITDA, sector, industry from yfinance Ticker.info). Uses `symbol_mapper` for ticker resolution
