...
- **Price Spike Filter:** `_filter_price_spikes()` in `data_fetcher.py` — three-stage data quality filter for BIST stocks: (1) Bimodal detection (catches interleaved nominal/adjusted prices, ratio > 5.0 triggers minority group removal), (2) Reversion-based outlier detection (gap=[1,2,3,5,7,10], checks if price deviates >50% from both sides, 8 iterative passes to catch multi-day spike blocks), applied in `fetch_backtest_data` and `_enrich_via_yahoo`.
