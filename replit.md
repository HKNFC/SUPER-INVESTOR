...
- `backtest_engine.py` — Backtest engine: replays screening strategy using disk-cached OHLCV data (no API fundamentals calls). Uses `fetch_backtest_data` for cache-first symbol loading with incremental updates, `get_cached_benchmark` for cached benchmark index. Point-in-time truncation (no look-ahead bias), recomputes momentum + scores at each rebalance, supports 1w/15d/1m rebalance frequency, computes equity curve, drawdown, Sharpe, alpha, volatility
